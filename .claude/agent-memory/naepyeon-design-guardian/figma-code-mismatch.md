...
**CHAT COMPOSER REDESIGN (2026-06-29):** Step2/3 chat input rewired to Figma 56:8418. `.chat-composer-box` is now a vertical stack: textarea (top) + `.chat-composer-actions` bar (bottom) holding attach(upload icon) + expand(chevronD) + spacer + send(arrowUp). The OLD `.chat-composer-footer` + `.chat-expand-btn` (expand button OUTSIDE the box) were REMOVED from create_screen.js; their CSS rules still linger in styles.css (orphaned, inert — left per no-blind-delete). New CSS added near old composer block: `.chat-composer-actions`, `.chat-composer-iconbtn`, `.chat-composer-send`, `.chat-attach-chip`(+`.chat-attach-name`/`.chat-attach-remove`), `.chat-composer-hint`. StepEdit gained `attachedFile` state + `fileInputRef` (UI-only file attach, NOT wired to revise API). `.create-pane-preview` got `border-radius:16px`+`box-shadow:var(--shadow-8)` (whole Step2/3 pane shadow). Evidence row (`.create-evidence-row` + `EvidenceUploader mode="preview" evidenceList=`) RESTORED in BOTH StepPreview and StepEdit below `.create-edit-layout` — it had been dropped (prop received, never rendered). StepEdit bottom "이전" button changed from `btn-subtle "이전 (미리보기)"` → `btn-primary "← 내용입력"` (chevronL white). StepPreview bottom-left still `btn-subtle "이전 (정보 수정)"` (unchanged, out of request scope).
